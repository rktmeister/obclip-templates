...
- Some longform images include a “caption” that starts with a URL (often a source/citation). The HTML for that caption is rendered as a separate Draft.js block, which becomes a standalone `[]()` link right after the image in Markdown. The template converts these `![Image](...) + [https://…](https://…)` sequences into Obsidian-style footnotes keyed by the image filename/media id (e.g. `[^img-G-zPg9_XYAAGEp-]`).
